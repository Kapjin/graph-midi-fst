"""Train and run Graph MuQ-FST on full audio tracks.
...
uses graph-augmented segment embeddings as the input sequence to FusionSegmentTransformer.
...
try:
    from audio_relation_graph_edges import build_audio_relation_graph
except ImportError as exc:
    raise SystemExit(
        "Could not import audio_relation_graph_edges.py. Put it in the same directory "
        "as this script."
    ) from exc



MUQ_FST_BUILD = "2026-08-27-graph-muq-audio-r1"
...
def _audio_graph_to_adjacency(
    graph: dict[str, np.ndarray],
    expected_length: int,
) -> torch.Tensor:
    adjacency = np.stack(
        [np.asarray(graph[relation], dtype=np.float32) for relation in GRAPH_RELATIONS],
        axis=0,
    )
    if adjacency.shape != (
        len(GRAPH_RELATIONS),
        ...
        raise ValueError(
            "Audio relation graph/MuQ segment count mismatch: "
            f"adjacency={adjacency.shape}, expected="
            f"({len(GRAPH_RELATIONS)}, {expected_length}, {expected_length})"
        )

    # GAT always has a valid self-neighbor even when a relation is sparse.
    for relation_index in range(len(GRAPH_RELATIONS)):
...
def build_graph_song_cache(
    ...
    for position, record in enumerate(embedding_records, start=1):
        ...
        key = cache_key(
            source,
            (
                "graph-muq-audio-relations-v1|"
                f"segment={SEGMENT_SECONDS:.8g}|sr={SAMPLE_RATE}|"
        ...
        try:
            ...
            if expected_length < 1:
                raise ValueError("Cached MuQ embedding sequence is empty")

            if not graph_path.is_file():
                graph = build_audio_relation_graph(
                    source,
                    sample_rate=SAMPLE_RATE,
                    segment_seconds=SEGMENT_SECONDS,
                ...
                adjacency = _audio_graph_to_adjacency(
                    graph,
                    expected_length=expected_length,
                )
                torch.save(
                    {
                        "format": "graph-muq-fst-relation-cache-v1",
                        "source": str(source.resolve()),
        ...
        if position == len(embedding_records) or position % 100 == 0:
            print(
                f"Audio relation graph cache: {position}/{len(embedding_records)} files",
                flush=True,
            )

    if not output_records:
        raise RuntimeError("No aligned audio relation-graph song records were produced")
    return output_records, failures
...
def collate_graph_song_embeddings(
    ...
    padded_embeddings = torch.zeros(
        (len(embeddings), MAX_SEGMENTS, MUQ_DIM),
        dtype=torch.float32,
    )
    padded_adjacency = torch.zeros(
        (len(embeddings), len(GRAPH_RELATIONS), MAX_SEGMENTS, MAX_SEGMENTS),
        dtype=torch.float32,
    )
    padding_mask = torch.ones(
        (len(embeddings), MAX_SEGMENTS),
        dtype=torch.bool,
    )
    ...
    for index, (sequence, adjacency) in enumerate(zip(embeddings, adjacencies)):
        ...
        if adjacency.shape[1] < length or adjacency.shape[2] < length:
            raise ValueError("Adjacency is shorter than its MuQ embedding sequence")
...
class RelationGATBranch(nn.Module):
    """Dense multi-head GAT over one explicit audio relation."""
    ...
    def forward(
        ...
        edge_mask = (
            edge_mask
            & valid_node.unsqueeze(-1)
            & valid_node.unsqueeze(-2)
        )

        # Preserve relation strength as a logarithmic attention bias.
        edge_bias = torch.log(
            positive_adjacency.clamp_min(1e-6)
        ).unsqueeze(1)
...
class MultiRelationGATEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.fusion = nn.Linear(
            relation_dim * len(GRAPH_RELATIONS),
            input_dim,
        )
    ...
    def forward(
        ...
        relation_outputs = [
            branch(
                embeddings,
                adjacency[:, relation_index],
                padding_mask,
            )
            for relation_index, branch in enumerate(self.branches)
        ]

        graph_message = torch.cat(
            relation_outputs,
            dim=-1,
        )
        graph_message = self.dropout(
            self.activation(
                self.fusion(graph_message)
            )
        )

        # H' = LayerNorm(H + graph_message)
        augmented = self.layer_norm(
            embeddings + graph_message
        )
...
class GraphAugmentedMuQFST(nn.Module):
    def __init__(
        self,
        input_dim: int,
...
def graph_stage2_epoch(
    ...
    for embeddings, adjacency, labels, padding_mask in loader:
        ...
        total_loss += (
            float(loss.detach()) * labels.numel()
        )
        all_labels.extend(
            labels.long().detach().cpu().tolist()
        )
...
def command_train_stage2(args: argparse.Namespace) -> None:
    ...
    stage1_signature = file_signature(
        args.stage1_checkpoint
    )
    ...
    if failures:
        save_json(
            args.output_dir / "graph_audio_preprocess_failures.json",
            failures,
        )
        print(
            f"Skipped malformed/unusable audio files: {len(failures)}"
        )
    ...
    train_dataset = GraphSongEmbeddingDataset(
        train_records
    )
    validation_dataset = GraphSongEmbeddingDataset(
        validation_records
    )
    ...
    checkpoint_path = (
        args.output_dir / "graph_muq_fst_stage2_best.pt"
    )
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            if validation_metrics.loss < best_loss:
                ...
                torch.save(
                    {
                        "format": "graph-muq-fst-stage2-v1",
                ...
                if stale_epochs >= args.patience:
                    print(
                        f"Graph Stage 2 early stopping after {epoch} epochs"
                    )
                    break
    finally:
        monitor.close()
    ...
    print(
        f"Training log: {monitor.log_path.resolve()}"
    )
    print(
        "TensorBoard log dir: "
    ...
    print(
        f"Loss curve: {monitor.plot_path.resolve()}"
    )


def extract_embeddings_and_graph_for_one_audio(
    audio_path: Path,
    adapter: MuQAdapter,
    device: torch.device,
    extraction_batch_size: int,
    ...
    all_segments = split_waveform_into_segments(
        waveform
    )
    segments = all_segments[:MAX_SEGMENTS]
    if not segments:
        raise ValueError(
            "The audio file produced no usable 30-second segments"
        )
    ...
    with torch.no_grad():
        for start in range(
            ...
            embeddings.append(
                adapter(batch).float().cpu()
            )

    song_embeddings = torch.cat(
        embeddings,
        dim=0,
    )
    ...
    adjacency = _audio_graph_to_adjacency(
        graph,
        expected_length=int(song_embeddings.shape[0]),
    )

    return (
        song_embeddings,
        adjacency,
        int(song_embeddings.shape[0]),
        len(all_segments) > MAX_SEGMENTS,
    )
...
def _predict_one_audio(
    ...
    (
        ...
        length,
        truncated,
    ) = extract_embeddings_and_graph_for_one_audio(
        audio_path,
        adapter,
        device,
        extraction_batch_size,
    ...
    padded_embeddings[0, :length] = (
        song_embeddings.to(device)
    )
    padded_adjacency[
        0, :, :length, :length
    ] = adjacency.to(device)
    ...
    with torch.no_grad():
        ...
        ai_probability = float(
            torch.sigmoid(logit).cpu()
        )

    prediction_id = (
        1 if ai_probability >= threshold else 0
    )
    return {
        "path": audio_path,
        "prediction_id": prediction_id,
        "prediction": (
            "AI" if prediction_id == 1 else "Human"
        ),
...
def command_infer(args: argparse.Namespace) -> None:
    ...
    if stage2_payload.get("format") != "graph-muq-fst-stage2-v1":
        raise ValueError(
            f"Unsupported Graph MuQ Stage 2 checkpoint: {args.stage2_checkpoint}"
        )
    ...
    if stage2_payload.get("stage1_signature") != stage1_signature:
        raise ValueError(
            "Graph MuQ Stage 2 was trained with a different Stage 1 checkpoint. "
            "Use the matching --stage1-checkpoint."
        )
    if stage2_payload.get("muq_model_id") != args.muq_model:
        raise ValueError(
            "Graph MuQ Stage 2 was trained with a different MuQ model. "
            "Use the matching --muq-model."
        )

    model = GraphAugmentedMuQFST(
        input_dim=int(
            stage2_payload["input_dim"]
        ),
        graph_relation_dim=int(
            stage2_payload["graph_relation_dim"]
        ),
        graph_heads=int(
            stage2_payload["graph_heads"]
        ),
        graph_dropout=float(
            stage2_payload["graph_dropout"]
        ),
        fst_hidden_dim=int(
            stage2_payload["fst_hidden_dim"]
        ),
        fst_num_heads=int(
            stage2_payload["fst_num_heads"]
        ),
        fst_num_layers=int(
            stage2_payload["fst_num_layers"]
        ),
        fst_dropout=float(
            stage2_payload["fst_dropout"]
        ),
    ).to(device)

    model.load_state_dict(
        stage2_payload["model_state"]
    )
    model.eval()

    recurrence_threshold = float(
        stage2_payload["recurrence_threshold"]
    )
    graph_top_k = int(
        stage2_payload["graph_top_k"]
    )
    threshold = float(
        stage2_payload.get("threshold", 0.5)
    )
    ...
    if args.audio is not None:
        ...
        print(
            f"Audio: {result['path'].resolve()}"
        )
        print(
            f"Prediction: {result['prediction']}"
        )
        print(
            f"AI probability: {result['ai_probability']:.6f}"
        )
        print(
            f"Human probability: {result['human_probability']:.6f}"
        )
        print(
            f"30-second segments used: {result['segments_used']}"
        )
        if result["truncated"]:
            print(
                f"Note: only the first {MAX_SEGMENTS} segments were used"
            )
        return

    if args.label is None:
        raise ValueError(
            "--label is required when using --audio-dir"
        )

    label_id = (
        1 if args.label == "ai" else 0
    )
    label_name = (
        "AI" if label_id == 1 else "Human"
    )
    audio_paths = discover_audio(
        args.audio_dir
    )
    ...
    if args.exclude_split is not None:
        excluded_manifest = load_manifest(
            args.exclude_split
        )
        excluded_paths = {
        ...
        excluded_count = (
            len(audio_paths) - len(filtered_paths)
        )
    ...
    if args.count is not None:
        ...
        if len(audio_paths) < args.count:
            raise ValueError(
                f"Requested --count {args.count}, but only {len(audio_paths)} "
                "audio files remain after exclusion"
            )
    ...
    print(
        f"Folder: {args.audio_dir.resolve()}"
    )
    print(
        f"Ground-truth label: {label_name}"
    )
    print(
        f"Audio files found: {total_found}"
    )
    if args.exclude_split is not None:
        print(
            f"Excluded by split manifest: {excluded_count}"
        )
        print(
            "Unseen audio files available: "
            f"{total_found - excluded_count}"
        )
    if args.count is not None:
        print(
            f"Target evaluated audio files: {args.count}"
        )
    ...
    for index, audio_path in enumerate(
        ...
        try:
            ...
            print(
                f"[{index}/{len(audio_paths)}] "
                f"{audio_path.name} | ERROR: {exc}",
                flush=True,
            )

    if evaluated == 0:
        raise RuntimeError(
            "No audio files could be evaluated"
        )
    ...
    if (
        ...
        raise RuntimeError(
            f"Requested {args.count} evaluated audio files, "
            f"but only {evaluated} could be evaluated after trying "
            f"{len(audio_paths)} unseen candidates"
        )
    ...
    print(
        f"Result summary saved: {result_file.resolve()}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=f"MuQ + {GRAPH_DISPLAY_NAME} + Fusion Segment Transformer for full-audio AI music detection"
    )
    ...
    stage2 = subparsers.add_parser(
        "train-stage2",
        help=f"Freeze Stage-1 MuQ and train {GRAPH_DISPLAY_NAME} + FST on audio segments",
    )
