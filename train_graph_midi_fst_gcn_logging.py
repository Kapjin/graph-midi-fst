"""GCN variant of train_graph_midi_fst_logging.py.

All non-graph behavior is shared with the GAT entry point. Only the graph
encoder is replaced with PyTorch Geometric's GCNConv.
"""
...
import train_graph_midi_fst_logging as base


class RelationGCNBranch(nn.Module):
    """One weighted PyG GCN branch for a single MIDI relation."""
    ...
    def forward(self, embeddings, adjacency, padding_mask):
        outputs = []
        for node_features, weights, mask in zip(embeddings, adjacency, padding_mask):
...
class MultiRelationGCNEncoder(nn.Module):
    ...
    def __init__(
        self,
        input_dim=base.MUSICBERT_BASE_DIM,
        relation_dim=256,
        num_heads=4,
        dropout=0.1,
    ) -> None:
...
base.MIDI_FST_BUILD = "2026-08-27-relation-gcn-fst-r1"
