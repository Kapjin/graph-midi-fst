#!/usr/bin/env python3
"""Train and run an MLP on whole-song symbolic MIDI features.
...
train
    Extract whole-song MIDI features, choose features according to the config
    below, fit train-only median imputation and standardization, and train MLP.
...
MIDI_FEATURE_MLP_BUILD = "2026-08-26-global-midi-feature-mlp-v10-logging"
MIDI_EXTS = {".mid", ".midi"}
...
RESULT_LOG_PATH = Path(__file__).resolve().parent / "midi_feature_mlp_results_2608271337.txt"
...
def vector_for_one_midi(
    ...
    return transformed, {key: float(value) for key, value in feature_dict.items()}
...
def predict_one_midi(
    ...
    features, raw_feature_dict = vector_for_one_midi(midi_path, payload)
