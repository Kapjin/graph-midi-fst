"""R-GCN variant of train_graph_midi_fst_logging.py.

All non-graph behavior is shared with the GAT entry point. The four MIDI
...
class MultiRelationRGCNEncoder(nn.Module):
    """A PyG R-GCN whose edge types are the four explicit MIDI relations."""
    ...
    def forward(self, embeddings, adjacency, padding_mask):
        ...
        for node_features, relations, mask in zip(
            ...
            for relation_id in range(len(base.GRAPH_RELATIONS)):
                weights = relations[
                    relation_id, :node_count, :node_count
                ]
...
base.MIDI_FST_BUILD = "2026-08-27-rgcn-fst-r1"
